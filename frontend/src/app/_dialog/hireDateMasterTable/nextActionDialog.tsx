...
type NextActionDialogProps = {
  ...
  setMakeNewArray: React.Dispatch<React.SetStateAction<MHireDate[]>>;
};
...
const NextActionDialog: React.FC<NextActionDialogProps> = (props) => {
  ...
  const saveValue = async (): Promise<void> => {
    setIsLoading(true);
    if (content.every((d) => d.sort !== null && d.hireDate !== null && d.retirementDate !== null)) {
      try {
        ...
        if (!hasDuplicate) {
          dispatch(setCreateHireDateContent(content));
        }
      } catch (error) {
        console.error(error);
      } finally {
