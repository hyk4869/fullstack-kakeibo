...
import { MCompany, setCreateCompanyContent } from '../../_store/slice';
...
type NextActionDialogProps = {
  ...
  setMakeNewArray: React.Dispatch<React.SetStateAction<MCompany[]>>;
};
  ...
  const saveValue = async (): Promise<void> => {
    ...
    if (content.every((d) => d.id !== null && d.name !== null && d.majorSector !== null)) {
      try {
        ...
        if (!hasDuplicate) {
          dispatch(setCreateCompanyContent(content));
        }
      } catch (error) {
        console.error(error);
      } finally {
