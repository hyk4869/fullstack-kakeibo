import { PrismaClient, TTax, TTaxBonus } from '@prisma/client';
import * as fs from 'fs';
import * as path from 'path';
import * as csvParser from 'csv-parser';
import { TSalaryTaxSampleData } from 'sampledata/types/MCategoryTypes';

const prisma = new PrismaClient();

const csvFilePath = path.resolve(__dirname, '../sampleCSV/TTax.csv');

const readCsvFile = async (filePath: string): Promise<TSalaryTaxSampleData[]> => {
  return new Promise((resolve, reject) => {
    const content: TSalaryTaxSampleData[] = [];
...
/** TTaxBonusのテストデータ */
export const createTTaxSeedData = async (): Promise<TTax[]> => {
  ...
  for (const data of csvData) {
    ...
    const createPosts = prisma.tTax.create({
      data: {
        sort,
